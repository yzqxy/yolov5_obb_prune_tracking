...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus, CrossConv,
                 BottleneckCSP, C3, C3TR, C3SPP, C3Ghost,ECA,C2f,SEModel,C2f_SE]:
            c1, c2 = ch[f], args[0]
...
def parse_pruned_model(maskbndict, d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv]:
            ...
            # if named_m_base=='model.0':
            #     bnc=int(maskbndict[named_m_bn.replace(".bn", ".conv.bn")].sum())
            # else:
            ...
            for p in range(n):
                ...
                # print('named_m_bottle_cv1_bn',named_m_bottle_cv1_bn)
                # print('named_m_bottle_cv2_bn',named_m_bottle_cv2_bn)
                bottle_cv1in = chin[-1]

                # print('bottle_cv1in',bottle_cv1in)

                bottle_cv1out = int(maskbndict[named_m_bottle_cv1_bn].sum())
                bottle_cv2out = int(maskbndict[named_m_bottle_cv2_bn].sum())
                # print('bottle_cv1out',bottle_cv1out)
                # print('bottle_cv2out',bottle_cv2out)
            ...
            named_m_cv1_bn = named_m_base + ".cv1.bn"
            named_m_cv2_bn = named_m_base + ".cv2.bn"
            # print('named_m_cv1_bn',named_m_cv1_bn)
            # print('named_m_cv2_bn',named_m_cv2_bn)
            cv1in = ch[f]
