...
class Tracker:
    ...
    def _match(self, detections):
        ...
        #如果框是is_confirmed状态，既track里的state=2，则放到confirmed_tracks队列中，否则放入unconfirmed_tracks中进行匹配，所以一帧没匹配上的框
        #会绘制，第二帧还没匹配上就不绘制了，一直存储在unconfirmed_tracks，直到30帧没匹配上再删除
        confirmed_tracks = [
    ...
    def _initiate_track(self, detection, class_id, conf):
        self.tracks.append(Track(
            detection.to_xyah(), detection.poly, self._next_id, class_id, conf, self.n_init, self.max_age, self.ema_alpha,
            detection.feature))
        self._next_id += 1
