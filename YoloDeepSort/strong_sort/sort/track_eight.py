...
import cv2
import numpy as np
from ..sort.kalman_filter_eight import KalmanFilter
...
class Track:
    ...
    def __init__(self, detection, poly, track_id, class_id, conf, n_init, max_age, ema_alpha,
                 feature=None):
        ...
        self.mean, self.covariance = self.kf.initiate(poly)
    ...
    def update(self, detection, class_id, conf):
        ...
        self.conf = conf
        self.class_id = class_id
        self.poly = detection.poly
        self.mean, self.covariance = self.kf.update(self.mean, self.covariance, detection.poly, detection.confidence)
