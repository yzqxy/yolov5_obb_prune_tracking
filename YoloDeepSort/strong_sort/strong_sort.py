...
class StrongSORT(object):
    ...
    def update(self, bbox_xywh, polys, confidences, classes, ori_img):
        self.height, self.width = ori_img.shape[:2]
        # generate detections
        features = self._get_features(bbox_xywh, ori_img)
        bbox_tlwh = self._xywh_to_tlwh(bbox_xywh)
        detections = [Detection(bbox_tlwh[i], polys[i], conf, features[i]) 
        ...
        for track in self.tracker.tracks:
            ...
            poly = track.poly
            # print('poly',poly)
            # box  = self._tlwh_to_xyxy(tlwh)
            # print('box',box)
            track_id = track.track_id
            class_id = track.class_id
            conf = track.conf
            outputs.append(np.array([tlwh, poly, track_id, class_id, conf]))
        if len(outputs) > 0:
