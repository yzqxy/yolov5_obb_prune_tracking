...
def rbox2poly2(obboxes,cls):
    ...
    if isinstance(obboxes, torch.Tensor):
        ...
        for i in range(len(obboxes)):
            ...
            if int(cls[i]) in cls_name:
                print(int(cls[i]))
                x,y, w, h = obboxes[i, :1], obboxes[i, 1:2],obboxes[i, 2:3], obboxes[i, 3:4] 

                point1 = torch.cat((x-w/2,y-h/2), dim=-1)
                point2 = torch.cat((x+w/2,y-h/2), dim=-1)
                point3 = torch.cat((x+w/2,y+h/2), dim=-1)
                point4 = torch.cat((x-w/2,y+h/2), dim=-1)
                point=torch.cat((point1, point2, point3, point4), dim=-1).cpu().numpy().tolist()
                point_list.append(point)

            else:
                center, w, h, theta = obboxes[i, :2], obboxes[i, 2:3], obboxes[i, 3:4], obboxes[i, 4:5]
                ...
                point=torch.cat((point1, point2, point3, point4), dim=-1).cpu().numpy().tolist()
                point_list.append(point)
        point_list=torch.Tensor(point_list).to("cuda:0")
