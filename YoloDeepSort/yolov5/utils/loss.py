...
class ComputeLoss:
    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        tcls, tbox, indices, anchors, tgaussian_theta,ttheat = self.build_targets(p, targets)  # targets
        # print('tgaussian_theta',tgaussian_theta)
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                # print('t',t.shape[0])
                #找到带旋转角度类别的正样本和不带旋转角度类别的正样本索引，目前的类别0带角度，所以找下标为0的，根据你自己的数据集格式来修改，后续来优化
                # rotota_obj_index_list=[]
                # for obj_i in range(t.shape[0]):
                #     if t[obj_i,0]==1:
                #         rotota_obj_index_list.append(obj_i)
                    
                # no_rotota_obj_index_list=[]
                # for obj_j in range(t.shape[0]):
                #     if t[obj_j,0]!=1:
                #         no_rotota_obj_index_list.append(obj_j)   

                # print('rotota_obj_index_list',rotota_obj_index_list)

                # theta Classification by Circular Smooth Label
                t_theta = tgaussian_theta[i].type(ps.dtype) # target theta_gaussian_labels


                # if len(no_rotota_obj_index_list)>0:
                #     no_ps2=[]
                #     no_t_theta2=[]
                #     for no_obj_index in range(len(no_rotota_obj_index_list)):
                #         a=ps[no_rotota_obj_index_list[no_obj_index], class_index:].unsqueeze(0)
                #         no_ps2.append(a)
                #         b=t_theta[no_rotota_obj_index_list[no_obj_index],:].unsqueeze(0)
                #         no_t_theta2.append(b)
                #     no_rotato_ps = torch.cat(no_ps2,dim=0)
                #     no_rotato_t_theta=torch.cat(no_t_theta2,dim=0)
                #     ltheta += self.BCEtheta(no_rotato_ps, no_rotato_t_theta)*0.000001

                # if len(rotota_obj_index_list)>0:
                #     ps2=[]
                #     t_theta2=[]
                #     for obj_index in range(len(rotota_obj_index_list)):
                #         a=ps[rotota_obj_index_list[obj_index], class_index:].unsqueeze(0)
                #         ps2.append(a)
                #         b=t_theta[rotota_obj_index_list[obj_index],:].unsqueeze(0)
                #         t_theta2.append(b)
                #     rotato_ps = torch.cat(ps2,dim=0)
                #     rotato_t_theta=torch.cat(t_theta2,dim=0)

                #     ltheta += self.BCEtheta(rotato_ps, rotato_t_theta)
