...
from utils.general import (LOGGER, box_iou, check_dataset, check_img_size, check_requirements, check_yaml,
                           ...
                           scale_coords, scale_polys, xywh2xyxy, xyxy2xywh, non_max_suppression_obb,scale_coords_xywh)
...
def run(data,
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        for si, pred in enumerate(out): # pred (tensor): (n, [xylsθ, conf, cls])
            ...
            if single_cls:
                pred[:, 6] = 0
            scale_coords_xywh(im[si].shape[1:], pred[:, :4], shape, shapes[si][1])

             # Evaluate
            if nl:
                tbox=labels[:, 1:5]
                scale_coords_xywh(im[si].shape[1:], tbox, shape, shapes[si][1])  # native-space labels
